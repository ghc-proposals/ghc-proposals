...
thus may not yet be as stable as we want them for ``GHC20xx``.
...
As `GeneralisedNewtypeDeriving` is in GHC2021 language, so should be `RoleAnnotations`. They are different sides of the same feature: without correct role annotations GND cannot be used safely.

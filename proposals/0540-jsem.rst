...
As far as the GHC jobclient implementation is concerned, we have identified
two salient points:

A. There is a hidden cost in having a GHC program (such as ``ghc`` itself) run on
   ...
   adverse effects: we propose that GHC will limit the rate at which it can
   release tokens to the semaphore so as to reduce this thrashing.
   The exact mechanism of rate limiting is unspecified here.

B. We believe that GHC invocations should not be too eager to release
...
2. Next, we compile the ``Bot`` unit, which is a large unit, with many modules,
   ...
   b. This invocation of ``ghc`` notices it has a lot of work to do (many modules
      ...
      semaphore. As no other processes are competing for semaphore tokens, this
      GHC invocation obtains the remaining 7 tokens.
...
Support for multiple home units (not yet fully implemented in ``cabal``)
would provide an alternative way to saturate the number of available capabilities.

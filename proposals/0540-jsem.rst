...
resources, so we propose a minimum delay between acquiring a resource and
...
We can also imagine ``cabal`` or ``stack`` being jobclients, if there is a
parent jobserver. For example, this would happen if ``nix`` manages the
semaphore, with ``nix`` spawning ``cabal`` subprocesses, and those ``cabal``
subprocesses spawning ``ghc`` subprocesses in turn.
...
system that one might be using (such as ``nix``), so it supports a wider range
of use cases.

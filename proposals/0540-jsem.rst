...
through the driver. If ``-jsem`` is not passed, GHC goes back to using the
``-jN`` flag to determine the number of jobs to run concurrently, and we don't
use the jobclient protocol. If ``-jsem=<sem>`` is passed but the semaphore
``<sem>`` does not exist, GHC should report an error.
...
Other interactions of the ``-jsem`` flag:

  - If ``-jsem`` is not passed to GHC, we fall back to the existing mechanisms
    for concurrency.
  - The ``-jsem`` and ``-j`` flags override eachother.
  ...
  - If GHC can't acquire any tokens from the semaphore, compilation will proceed
    serially (as if running with ``-j1``). This is because we always have the
    one implicit token, and because resource acquisition is done in parallel, so
    we don't block the world just because we are indefinitely waiting on the
    semaphore.

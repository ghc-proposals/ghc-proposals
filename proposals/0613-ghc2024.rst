...
primarily because of ``MonoLocalBinds``.  Cabal packages will
specify a ``default-language`` (or fall back on ``Haskell98``) and hence will be unaffected.
But some users compile programs using ``ghc`` directly, so a change in the
...
* GHC 9.10 will add support for ``GHC2024``, but the default language edition
  will remain ``GHC2021`` (in both ``ghc`` and ``ghci``) for the time being.

* Any future change to the default language edition, in the light of experience
  with ``GHC2024``, will require a separate proposal. As with all backwards
  incompatible proposals this should include appropriate warning and deprecation
  periods.

* GHC may introduce a warning (controlled by
  ``-Wunspecified-language`` and in ``-Wcompat``) when compiling a ``.hs``

...
* The ``%1`` modifier is handled as a special case. It's renamed (and so
  typechecked) the same as ``%One`` (using the ``One`` from ``base``), even if
  it appears somewhere that linear modifiers aren't expected. If a user does
  want the modifier ``1 :: Nat``, they can write it as ``%01``. (Requires
  ``-XDataKinds``.)
...
type ``1 :: Nat`` and requires ``-XDataKinds``. The warning generated by
``-Wunknown-modifiers`` hints that the user probably wants to enable
``-XLinearTypes``.

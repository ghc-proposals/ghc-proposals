...
Existentially bound type and dictionary variables can be supported with some
difficulty. Simplest cases like ::
...
are supported even by the simplest desugarer that just generates a join point
(see "desugaring" section) with arguments for bound variables in patterns.
This is the desugared form generated by the prototype implementation: ::
...
However, more complex uses of existentials are tricky to desugar in this simple
desugaring strategy. For example, this could be supported by not passing unused
binders to the join point: ::

    data E
      = forall x . Eq x => Eq x
      | forall x . Ord x => Ord x
